...
students3_tidy <- students3 %>%
  gather(key = "class", value = "score", class1:class5, na.rm = TRUE) %>% 
...
```{r}
library(readr)
students3_tidy %>% mutate(class = parse_number(class)) %>% head()
```
...
```{r}
failed <- students4 %>%
  filter(final %in% c("C", "D", "E", "F")) %>%
  mutate(status = "failed")

passed <- students4 %>%
  filter(final %in% c("A", "B")) %>%
  mutate(status = "passed")

students_status <- bind_rows(failed, passed)
head(students_status)
```
...
```{r}
sat_tidy %>% ggplot(aes(x = score_range, y = prop, fill = sex)) +
...
```{r}
update(this_moment, hours = 21, minutes = 32, seconds = 21)
```
...
llegada_hk
fecha <- ymd("2008-06-17", tzone = "Asia/Singapore")
fecha
intervalo <- interval(fecha, llegada_hk)
intervalo
